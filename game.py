...
player_hp, player_gold, player_inventory, town_x, town_y, play_game = gamefunctions.gamestart()

#Main Game Loop
while play_game == True:
    move = gamefunctions.town_menu(player_hp, player_gold) #Gets player move
    if move == 1: # If player move is one then the player is sent to the map (If map is closed abruptly then the game loop shutrs off)
        player_hp, player_gold, player_inventory, play_game = gamefunctions.mapUsage(player_hp, player_gold, player_inventory, town_x, town_y, town_x, town_y)
